...
(define-private (is-undercollateralized (vault-data {
    ...
    (let (
            ...
            (total-debt (+ (get borrowed-amount vault-data)
                (get interest-accumulated vault-data)
            ))
        )
        (if (is-err collateral-value-result)
            ...
            (let (
                ...
                (if (is-err min-collateral-needed-result)
                    ...
                    (< collateral-value
                        (unwrap-panic min-collateral-needed-result)
                    )
                )
            )
        )
                ...
                (let (
                    ...
                    (if (is-err max-borrow-result)
                        ...
                        (let ((max-borrow (unwrap-panic max-borrow-result)))
                            (asserts!
                                (<= (+ amount-to-borrow total-debt) max-borrow)
            ...
            (let (
                ...
                (let ((total-debt (+ (get borrowed-amount updated-vault)
                    ...
                    (let ((effective-repayment (if (> amount-to-repay total-debt)
                        ...
                        (let (
                                (interest-payment (if (> (get interest-accumulated updated-vault)
                                        effective-repayment
                                    )
                                    effective-repayment
                                    (get interest-accumulated updated-vault)
                            ...
                            (let ((fee-amount (mul-div interest-payment
                                    (var-get protocol-fee-rate) u100
                                )))
                                ;; Update protocol statistics
                                (var-set total-fees-collected
                                    (+ (var-get total-fees-collected)
                                        (unwrap-panic fee-amount)
                                    ))
                                (var-set total-borrowed
    ...
    (let (
        ...
        (begin
            ...
            (let (
                ...
                (asserts!
                    (<= amount-to-withdraw (get collateral-amount updated-vault))
                    ERR_INSUFFICIENT_COLLATERAL
                )
                (let (
                        (new-collateral-amount (- (get collateral-amount updated-vault)
                            amount-to-withdraw
                        ))
                        (total-debt (+ (get borrowed-amount updated-vault)
                            (get interest-accumulated updated-vault)
                        ...
                        (let ((collateral-value-result (calculate-collateral-value new-collateral-amount)))
                            (if (is-err collateral-value-result)
                                ...
                                (let (
                                        ...
                                        (min-collateral-needed (mul-div total-debt
                                    ...
                                    (asserts!
                                        (>= new-collateral-value
                                            (unwrap-panic min-collateral-needed)
                                        )
            ...
            (let (
                ...
                (let (
                    ...
                    (if (is-err collateral-value-result)
                        ...
                        (let (
                                (collateral-value (unwrap-panic collateral-value-result))
                                (liquidation-amount-result (mul-div total-debt
                                    (+ u100 (var-get liquidation-penalty))
                                    u100
                                ))
                            )
                            (if (is-err liquidation-amount-result)
                                ...
                                (let ((liquidation-amount (unwrap-panic liquidation-amount-result)))
                                    ...
                                    (let ((btc-to-liquidate-result (if (> liquidation-amount
                                                collateral-value
                                            )
                                        ...
                                        (if (is-err btc-to-liquidate-result)
                                            ...
                                            (let ((btc-to-liquidate (unwrap-panic btc-to-liquidate-result)))
                                                ...
                                                (let ((liquidator-bonus-result (mul-div btc-to-liquidate
                                                        (var-get liquidation-penalty)
                                                        u100
                                                    )))
                                                    (if (is-err liquidator-bonus-result)
                                                        ...
                                                        (let ((liquidator-bonus (unwrap-panic liquidator-bonus-result)))
                                                            ...
                                                            (let ((borrowed-amount-reduction-result (if (>
                                                                        liquidation-amount
                                                                        total-debt
                                                                    )
                                                                    (ok (get
                                                                ...
                                                                (if (is-err borrowed-amount-reduction-result)
                                                                    ...
                                                                    (let ((borrowed-amount-reduction (unwrap-panic borrowed-amount-reduction-result)))
                                                                        ...
                                                                        (let ((interest-reduction-result (if (>
                                                                                    liquidation-amount
                                                                                    total-debt
                                                                                )
                                                                                (ok (get
